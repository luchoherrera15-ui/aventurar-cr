...
import type { GrupoLealtad } from "./shell-lealtad";
...
/**
 * ════════════════════════════════════════════════════════════════════
 *  EL ARMADO DEL PANEL DE LEALTAD — datos y secciones, sin el chrome
 * ════════════════════════════════════════════════════════════════════
 *
 * Hasta el 7 sep 2026 esto era la página /lealtad/panel/[id] entera.
 * El dueño pidió «todas las funciones de Lealtad, absolutamente todo,
 * dentro del menú Lealtad del panel de Linksy». La única forma de dar
 * TODO sin duplicar 1.500 líneas es que el armado sea uno y los
 * envoltorios sean dos: `page.tsx` lo viste con `ShellLealtad` (el
 * rail navy de Lealtad) y `solutions/panel/[id]/lealtad` lo viste con
 * el marco de Linksy. Los datos, los permisos, las secciones y sus
 * enlaces internos (`#clientes`, `#plan`…) son exactamente los mismos.
 *
 * Devuelve un resultado y no renderiza: `redirect` lo hace el
 * envoltorio (un redirect adentro de una función compartida dejaría al
 * panel de Linksy sin decidir a dónde), y las pantallas de antesala
 * (en revisión, sin complemento) viajan como nodo.
 */
export type PanelLealtadArmado =
  | { tipo: "redirect"; destino: string }
  | { tipo: "pantalla"; nodo: React.ReactNode }
  | {
      tipo: "panel";
      negocio: { nombre: string; plan: string | null };
      usuario: { nombre: string; email: string; rol: string };
      grupos: GrupoLealtad[];
      contenidos: Record<string, React.ReactNode>;
      /** Envuelve lo que se pinte con el contexto del programa. */
      envolver: (nodo: React.ReactNode) => React.ReactNode;
    };

export async function armarPanelLealtad({
  params,
  searchParams,
  ocultar = [],
}: {
  /**
   * Secciones que el envoltorio no quiere (7 sep 2026): el panel de
   * Linksy oculta «Mi página» de Lealtad porque Linksy ya tiene la suya.
   * Se quitan ANTES de calcular `visibles`, así los enlaces internos a
   * esa sección (`ancla`) también desaparecen, no apuntan a la nada.
   */
  ocultar?: string[];
  ...
  if (!acceso.user) return { tipo: "redirect", destino: "/lealtad/login" };
  if (!acceso.ok) return { tipo: "redirect", destino: "/lealtad/panel" };
  ...
  if (!rancho) return { tipo: "redirect", destino: "/lealtad/panel" };
  ...
  for (const g of grupos) g.items = g.items.filter((i) => !ocultar.includes(i.id));
  ...
  return {
    ...
    contenidos,
    envolver: (nodo) => (
      <ProveedorPrograma ranchoId={id} programaInicial={p} recompensasIniciales={lista} tieneCercania={cercania === true}>
        {nodo}
      </ProveedorPrograma>
    ),
  };
