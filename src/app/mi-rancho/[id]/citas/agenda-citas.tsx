...
type Miembro = { id: string; nombre: string; tipo: string; activo: boolean };
type Servicio = {
  id: string;
  nombre: string;
...
export default function AgendaCitas({
  ...
  const [vista, setVista] = useState<"lista" | "personas">("lista");
  ...
  const columnas: { id: string | null; nombre: string }[] = [
    ...equipo
      .filter((m) => m.activo || conCitasDelDia.has(m.id))
      .map((m) => ({ id: m.id as string | null, nombre: m.nombre })),
    ...(haySinAsignar || equipo.filter((m) => m.activo).length === 0
      ? [{ id: null, nombre: equipo.length > 0 ? "Sin asignar" : "El negocio" }]
      : []),
  ];
      ...
      {vista === "personas" && citas.length > 0 && (
        <div className="overflow-x-auto rounded-2xl border border-aventurea-line bg-aventurea-surface p-4">
            ...
            {columnas.map((col) => {
              ...
              return (
                <div key={col.id ?? "sin"} className="w-[160px] shrink-0 sm:w-[190px]">
                  <p className="mb-1 truncate text-center text-[12px] font-bold text-aventurea-navy">
                    {col.nombre}
                  </p>
                  <div
                    className="relative rounded-xl border border-aventurea-line bg-aventurea-cream-2/60"
                    style={{ height: altoGrilla }}
                  >
                    {Array.from(
                      { length: (finGrilla - inicioGrilla) / 60 },
                      (_, i) => inicioGrilla + i * 60,
                    ).map((min) => (
                      <div
                        key={min}
                        className="absolute left-0 right-0 border-t border-aventurea-line/50"
                    ...
                    {bloqueosCol.map(({ bloqueo, rango }) => (
                      <div
                        ...
                        className="absolute left-1 right-1 rounded-lg bg-zinc-400/20"
                        style={{
          ...
          <p className="mt-2 text-[11.5px] text-zinc-500">
            Tocá una cita para ver el detalle y marcar asistencia.
          </p>
        </div>
      )}
    </div>
