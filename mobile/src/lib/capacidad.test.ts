...
/**
 * PORTADO de src/lib/agenda/capacidad.test.ts de /web.
 *
 * La concurrencia del recurso (`equipo_rancho.cupo_simultaneo`, 0109)
 * es justo lo que le faltaba al gemelo del app: sin ella, un negocio
 * con dos sillas o con clases grupales ofrecía menos horas desde el
 * teléfono que desde la compu. Estas pruebas son el candado.
 *
 * Lo que protegen NO es la funcionalidad nueva: es que la funcionalidad
 * nueva no le cambie NADA a los negocios que ya operan. La promesa del
 * diseño es "con cupo 1 el motor se comporta exactamente como antes".
 *
...
describe("las citas del negocio (sin miembro) estorban a todos los recursos", () => {
  it("con cupo 2, una cita sin miembro consume un cupo de cada recurso", () => {
    const { porRecurso } = calcularDisponibilidad(
      base({
        recursos: [sala(2)],
        citas: [{ miembroId: null, horaInicio: "09:00", duracionMinutos: 60, exclusiva: false }],
      }),
    );
    expect(porRecurso.sala).toContain("09:00");
