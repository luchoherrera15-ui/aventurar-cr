/**
 * El CRM de clientes de un negocio de citas en el app: espejo de
 * src/lib/crm-citas.ts de /web (misma base de Supabase, misma lógica
 * pura de fichas de cliente). Si cambiás algo acá, cambialo también
 * allá — son gemelos a mano porque el app no puede importar código del
 * proyecto Next.
 *
 * DERIVADO, NO DUPLICADO: no existe una tabla "clientes". La ficha de
 * cada cliente se calcula al vuelo desde sus reservas, así nunca se
 * desincroniza de la agenda real.
 *
 * IDENTIDAD DEL CLIENTE: una misma persona puede aparecer como cuenta
 * con sesión (cliente_id), como correo tecleado en una cita manual, o
 * solo como nombre en una agenda importada. La llave de agrupación es,
 ...
 * cambia, se cambia solo acá (y en su gemelo de /web).
 */
 ...
 * de afuera (fechaISOLocal o la zona del negocio) para que sea probable.
 */
